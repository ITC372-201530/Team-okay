...
//[ExecuteInEditMode]
public class ColourWheelV2 : Graphic 
{
	public int colour;
...
//		
		int intendedAngle = getIntendedAngle ();
		int currentAngle = (int)(rectTransform.localRotation.z);
		float t = Time.deltaTime*spd;

		int newAngle = (int)((intendedAngle-currentAngle) * t + currentAngle);
//		
...
//		rectTransform.anchoredPosition3D = new Vector3(0,0,intendedZ);
		rectTransform.localRotation = Quaternion.Euler(0,0,intendedAngle);
	}
	
	
	private int getIntendedAngle()
	{
	...
	protected override void OnFillVBO (List<UIVertex> vbo)
	{
		...
		float corner3 = (corner1.y+corner2.y)*0.5f;
		
		UIVertex vert = UIVertex.simpleVert;
		
		vert.position = new Vector2(corner1.x, corner3);
		vert.color = color;
		vbo.Add(vert);
